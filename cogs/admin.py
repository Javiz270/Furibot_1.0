...
class Admin(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        # Evita DM duplicado si se programa un nuevo mute antes de que acabe el anterior.
    ...
    def _schedule_mute_end_dm(self, user_id, guild_id, guild_name, segundos, reset_warns=False):
        """Tras `segundos`, limpia DB y envía DM al usuario (mute por warns o /mute)."""
    ...
    async def registrar_en_db(self, interaction, tipo, usuario, razon):
        """Envía la información a la tabla única 'infractions' en Stats"""
        if interaction.guild is None:
            return

        stats_cog = self.bot.get_cog('Stats')
        if stats_cog:
            # Aquí llamamos a la nueva función maestra log_infraction
            await stats_cog.log_infraction(
                guild_id=interaction.guild.id,
                guild_name=interaction.guild.name,
                tipo=tipo, # Ej: 'KICK', 'BAN', 'MUTE', 'WARN'
            ...
            if log_channel is None:
                print(f"⚠️ No se encontró el canal de logs configurado: {log_channel_id}")
                return
            ...
            try:
                ...
                print(f"⚠️ No se pudo enviar el embed de logs: {e}")

        else:
            print("⚠️ El cog Stats no está cargado; no se pudo registrar la infracción.")
    ...
    async def mute(self, interaction: discord.Interaction, usuario: discord.Member, minutos: int = 10, razon: str = "No especificada"):
        ...
        # Usamos el nuevo auxiliar e incluimos el tiempo en la razón
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        try:
            ...
            print(
                f"🔁 Reaplicado mute a {member.display_name} en {member.guild.name} "
